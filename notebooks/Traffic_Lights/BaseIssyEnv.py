...
class BaseIssyEnv1(Env):
    ...
    * action_spec: (dict<str,[str]>) allowed states for each traffic light ID.
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        network.net_params.osm_path = '/home/julien/projet_CIL4SYS/NOTEBOOKS/issy.osm'
...
class BaseIssyEnv2(Env):
    """
    Here, we observe all vehicles, in particular the queue length
    """
    ...
    def __init__(self, env_params, sim_params, network, simulator='traci'):
        ...
        self.all_vehicles_ids = self.k.vehicle.get_ids()
        self._init_tl_wait_steps()
        self.pos_veh = {}
        self.veh_wait_steps = {veh_id: 0 for veh_id in self.all_vehicles_ids}

        # Used for debug purposes
        self.current_timestep = 0


    def _update_pos_veh(self):
        "Return a dictionnary giving the edge on which each vehicle is on"
        ...
        for veh_id in new_pos_veh:
            if ":" in new_pos_veh[veh_id]:
                new_pos_veh[veh_id] = self.pos_veh[veh_id]
                
        self.pos_veh = new_pos_veh
        
    def _init_tl_wait_steps(self):
        """Initializes attributes that will store the number of steps stayed
        idle by the traffic lights"""
        ...
        # We instantiate a dictionary with tl_ids as keys and time steps spent
        # idled as values. We set all values to 0 since this is an init.
        self.tl_wait_steps = {tl_id: {'current_state': '','timer': 0} for tl_id in self._all_tl_names}
    ...
    def map_action_to_tl_states(self, rl_actions):
        ...
        for i, tl_id in enumerate(self.action_spec.keys()):
            current_state = self.k.traffic_light.get_state(tl_id)
            timer_value   = self.tl_wait_steps[tl_id]['timer']
            if timer_value < self.tl_constraint_min:
                new_state[i] = current_state
            else:
                # Pick new state if tl state hasn't changed in a while
                ...
                # Update state and timer if state changed
                if new_state[i] is not current_state:
                    self.tl_wait_steps[tl_id] = {'current_state': new_state[i],'timer': 0}
    ...
    def _update_wait_steps(self):
        """This method updates `self.veh_wait_steps`.

        Ex: If human_1 has been idled for 5 timesteps and human_2 is moving at
        1km/h, then `self.obs_veh_wait_steps` = {'human_1': 5, 'human_2': 0}"""

        for veh_id in self.all_vehicles_ids:
            # add keys if a vehicule appear
            if veh_id not in self.veh_wait_steps.keys():
                self.veh_wait_steps[veh_id] = 0
            
            # update according to speed value
            if self.k.vehicle.get_speed(veh_id):
                self.veh_wait_steps[veh_id] = 0
            else:
                self.veh_wait_steps[veh_id] = self.veh_wait_steps[veh_id] + 1

        # remove element from dictionnary if flow vehicule is gone
        veh_id_to_remove = list(set(self.veh_wait_steps.keys()) - set(self.all_vehicles_ids))
        for veh_id in veh_id_to_remove:
            del self.veh_wait_steps[veh_id]
        
    def _increment_tl_wait_steps(self):
        """This method increments `self.obs_tl_wait_steps`."""
        for tl_id in self.action_spec.keys():
            self.tl_wait_steps[tl_id]['timer'] += 1
    ...
    def additional_command(self):
        ...
        self.all_vehicles_ids = self.k.vehicle.get_ids().copy()
        self._increment_tl_wait_steps()
        self._update_wait_steps()
        self._update_pos_veh()
